...
type MailTemplatePreview struct {
	...
	ErrorMsg       *String    `xmlrpc:"error_msg,omitempty" json:"error_msg,omitempty"`
...
func (c *Client) CreateMailTemplatePreview(mtp *MailTemplatePreview) (int64, error) {
	ids, err := c.CreateMailTemplatePreviews([]*MailTemplatePreview{mtp})
	if err != nil {
		return -1, err
	}
	if len(ids) == 0 {
...
// CreateMailTemplatePreview creates a new mail.template.preview model and returns its id.
func (c *Client) CreateMailTemplatePreviews(mtps []*MailTemplatePreview) ([]int64, error) {
	var vv []interface{}
	for _, v := range mtps {
		vv = append(vv, v)
	}
	return c.Create(MailTemplatePreviewModel, vv, nil)
}
...
func (c *Client) UpdateMailTemplatePreviews(ids []int64, mtp *MailTemplatePreview) error {
	return c.Update(MailTemplatePreviewModel, ids, mtp, nil)
}
...
func (c *Client) FindMailTemplatePreviewIds(criteria *Criteria, options *Options) ([]int64, error) {
	return c.Search(MailTemplatePreviewModel, criteria, options)
}
