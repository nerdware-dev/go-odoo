...
func (c *Client) CreateAccountResequenceWizard(arw *AccountResequenceWizard) (int64, error) {
	ids, err := c.CreateAccountResequenceWizards([]*AccountResequenceWizard{arw})
	if err != nil {
		return -1, err
	}
	if len(ids) == 0 {
...
// CreateAccountResequenceWizard creates a new account.resequence.wizard model and returns its id.
func (c *Client) CreateAccountResequenceWizards(arws []*AccountResequenceWizard) ([]int64, error) {
	var vv []interface{}
	for _, v := range arws {
		vv = append(vv, v)
	}
	return c.Create(AccountResequenceWizardModel, vv, nil)
}
...
func (c *Client) UpdateAccountResequenceWizards(ids []int64, arw *AccountResequenceWizard) error {
	return c.Update(AccountResequenceWizardModel, ids, arw, nil)
}
...
func (c *Client) FindAccountResequenceWizardIds(criteria *Criteria, options *Options) ([]int64, error) {
	return c.Search(AccountResequenceWizardModel, criteria, options)
}
