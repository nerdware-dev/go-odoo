...
func (c *Client) CreateMailResendCancel(mrc *MailResendCancel) (int64, error) {
	ids, err := c.CreateMailResendCancels([]*MailResendCancel{mrc})
	if err != nil {
		return -1, err
	}
	if len(ids) == 0 {
...
// CreateMailResendCancel creates a new mail.resend.cancel model and returns its id.
func (c *Client) CreateMailResendCancels(mrcs []*MailResendCancel) ([]int64, error) {
	var vv []interface{}
	for _, v := range mrcs {
		vv = append(vv, v)
	}
	return c.Create(MailResendCancelModel, vv)
}
...
func (c *Client) FindMailResendCancel(criteria *Criteria) (*MailResendCancel, error) {
	...
	return nil, fmt.Errorf("mail.resend.cancel was not found with criteria %v", criteria)
}
...
func (c *Client) FindMailResendCancelId(criteria *Criteria, options *Options) (int64, error) {
	...
	return -1, fmt.Errorf("mail.resend.cancel was not found with criteria %v and options %v", criteria, options)
}
