...
func (c *Client) CreatePhoneBlacklistRemove(pbr *PhoneBlacklistRemove) (int64, error) {
	ids, err := c.CreatePhoneBlacklistRemoves([]*PhoneBlacklistRemove{pbr})
	if err != nil {
		return -1, err
	}
	if len(ids) == 0 {
...
// CreatePhoneBlacklistRemove creates a new phone.blacklist.remove model and returns its id.
func (c *Client) CreatePhoneBlacklistRemoves(pbrs []*PhoneBlacklistRemove) ([]int64, error) {
	var vv []interface{}
	for _, v := range pbrs {
		vv = append(vv, v)
	}
	return c.Create(PhoneBlacklistRemoveModel, vv, nil)
}
...
func (c *Client) UpdatePhoneBlacklistRemoves(ids []int64, pbr *PhoneBlacklistRemove) error {
	return c.Update(PhoneBlacklistRemoveModel, ids, pbr, nil)
}
...
func (c *Client) FindPhoneBlacklistRemoveIds(criteria *Criteria, options *Options) ([]int64, error) {
	return c.Search(PhoneBlacklistRemoveModel, criteria, options)
}
