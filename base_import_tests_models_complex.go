...
func (c *Client) CreateBaseImportTestsModelsComplex(btmc *BaseImportTestsModelsComplex) (int64, error) {
	ids, err := c.CreateBaseImportTestsModelsComplexs([]*BaseImportTestsModelsComplex{btmc})
	if err != nil {
		return -1, err
	}
	if len(ids) == 0 {
...
// CreateBaseImportTestsModelsComplex creates a new base_import.tests.models.complex model and returns its id.
func (c *Client) CreateBaseImportTestsModelsComplexs(btmcs []*BaseImportTestsModelsComplex) ([]int64, error) {
	var vv []interface{}
	for _, v := range btmcs {
		vv = append(vv, v)
	}
	return c.Create(BaseImportTestsModelsComplexModel, vv)
}
...
func (c *Client) FindBaseImportTestsModelsComplex(criteria *Criteria) (*BaseImportTestsModelsComplex, error) {
	...
	return nil, fmt.Errorf("base_import.tests.models.complex was not found with criteria %v", criteria)
}
...
func (c *Client) FindBaseImportTestsModelsComplexId(criteria *Criteria, options *Options) (int64, error) {
	...
	return -1, fmt.Errorf("base_import.tests.models.complex was not found with criteria %v and options %v", criteria, options)
}
