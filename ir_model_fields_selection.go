...
func (c *Client) CreateIrModelFieldsSelection(imfs *IrModelFieldsSelection) (int64, error) {
	ids, err := c.CreateIrModelFieldsSelections([]*IrModelFieldsSelection{imfs})
	if err != nil {
		return -1, err
	}
	if len(ids) == 0 {
...
// CreateIrModelFieldsSelection creates a new ir.model.fields.selection model and returns its id.
func (c *Client) CreateIrModelFieldsSelections(imfss []*IrModelFieldsSelection) ([]int64, error) {
	var vv []interface{}
	for _, v := range imfss {
		vv = append(vv, v)
	}
	return c.Create(IrModelFieldsSelectionModel, vv)
}
...
func (c *Client) FindIrModelFieldsSelection(criteria *Criteria) (*IrModelFieldsSelection, error) {
	...
	return nil, fmt.Errorf("ir.model.fields.selection was not found with criteria %v", criteria)
}
...
func (c *Client) FindIrModelFieldsSelectionId(criteria *Criteria, options *Options) (int64, error) {
	...
	return -1, fmt.Errorf("ir.model.fields.selection was not found with criteria %v and options %v", criteria, options)
}
