...
type ResPartnerTitle struct {
	...
	Salutation  *String   `xmlrpc:"salutation,omitempty" json:"salutation,omitempty"`
	Shortcut    *String   `xmlrpc:"shortcut,omitempty" json:"shortcut,omitempty"`
...
func (c *Client) CreateResPartnerTitle(rpt *ResPartnerTitle) (int64, error) {
	ids, err := c.CreateResPartnerTitles([]*ResPartnerTitle{rpt})
	if err != nil {
		return -1, err
	}
	if len(ids) == 0 {
