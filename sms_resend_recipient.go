...
type SmsResendRecipient struct {
	...
	SmsResendId    *Many2One  `xmlrpc:"sms_resend_id,omitempty" json:"sms_resend_id,omitempty"`
...
func (c *Client) CreateSmsResendRecipient(srr *SmsResendRecipient) (int64, error) {
	ids, err := c.CreateSmsResendRecipients([]*SmsResendRecipient{srr})
	if err != nil {
		return -1, err
	}
	if len(ids) == 0 {
...
// CreateSmsResendRecipient creates a new sms.resend.recipient model and returns its id.
func (c *Client) CreateSmsResendRecipients(srrs []*SmsResendRecipient) ([]int64, error) {
	var vv []interface{}
	for _, v := range srrs {
		vv = append(vv, v)
	}
	return c.Create(SmsResendRecipientModel, vv, nil)
}
...
func (c *Client) UpdateSmsResendRecipients(ids []int64, srr *SmsResendRecipient) error {
	return c.Update(SmsResendRecipientModel, ids, srr, nil)
}
...
func (c *Client) FindSmsResendRecipientIds(criteria *Criteria, options *Options) ([]int64, error) {
	return c.Search(SmsResendRecipientModel, criteria, options)
}
