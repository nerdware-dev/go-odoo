...
func (c *Client) CreateAccountReconcileModelPartnerMapping(armpm *AccountReconcileModelPartnerMapping) (int64, error) {
	ids, err := c.CreateAccountReconcileModelPartnerMappings([]*AccountReconcileModelPartnerMapping{armpm})
	if err != nil {
		return -1, err
	}
	if len(ids) == 0 {
...
// CreateAccountReconcileModelPartnerMapping creates a new account.reconcile.model.partner.mapping model and returns its id.
func (c *Client) CreateAccountReconcileModelPartnerMappings(armpms []*AccountReconcileModelPartnerMapping) ([]int64, error) {
	var vv []interface{}
	for _, v := range armpms {
		vv = append(vv, v)
	}
	return c.Create(AccountReconcileModelPartnerMappingModel, vv, nil)
}
...
func (c *Client) UpdateAccountReconcileModelPartnerMappings(ids []int64, armpm *AccountReconcileModelPartnerMapping) error {
	return c.Update(AccountReconcileModelPartnerMappingModel, ids, armpm, nil)
}
...
func (c *Client) FindAccountReconcileModelPartnerMappingIds(criteria *Criteria, options *Options) ([]int64, error) {
	return c.Search(AccountReconcileModelPartnerMappingModel, criteria, options)
}
