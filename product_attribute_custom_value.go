...
func (c *Client) CreateProductAttributeCustomValue(pacv *ProductAttributeCustomValue) (int64, error) {
	ids, err := c.CreateProductAttributeCustomValues([]*ProductAttributeCustomValue{pacv})
	if err != nil {
		return -1, err
	}
	if len(ids) == 0 {
...
// CreateProductAttributeCustomValue creates a new product.attribute.custom.value model and returns its id.
func (c *Client) CreateProductAttributeCustomValues(pacvs []*ProductAttributeCustomValue) ([]int64, error) {
	var vv []interface{}
	for _, v := range pacvs {
		vv = append(vv, v)
	}
	return c.Create(ProductAttributeCustomValueModel, vv)
}
...
func (c *Client) FindProductAttributeCustomValue(criteria *Criteria) (*ProductAttributeCustomValue, error) {
	...
	return nil, fmt.Errorf("product.attribute.custom.value was not found with criteria %v", criteria)
}
...
func (c *Client) FindProductAttributeCustomValueId(criteria *Criteria, options *Options) (int64, error) {
	...
	return -1, fmt.Errorf("product.attribute.custom.value was not found with criteria %v and options %v", criteria, options)
}
