...
func (c *Client) CreatePaymentAcquirerOnboardingWizard(paow *PaymentAcquirerOnboardingWizard) (int64, error) {
	ids, err := c.CreatePaymentAcquirerOnboardingWizards([]*PaymentAcquirerOnboardingWizard{paow})
	if err != nil {
		return -1, err
	}
	if len(ids) == 0 {
...
// CreatePaymentAcquirerOnboardingWizard creates a new payment.acquirer.onboarding.wizard model and returns its id.
func (c *Client) CreatePaymentAcquirerOnboardingWizards(paows []*PaymentAcquirerOnboardingWizard) ([]int64, error) {
	var vv []interface{}
	for _, v := range paows {
		vv = append(vv, v)
	}
	return c.Create(PaymentAcquirerOnboardingWizardModel, vv)
}
...
func (c *Client) FindPaymentAcquirerOnboardingWizard(criteria *Criteria) (*PaymentAcquirerOnboardingWizard, error) {
	...
	return nil, fmt.Errorf("payment.acquirer.onboarding.wizard was not found with criteria %v", criteria)
}
...
func (c *Client) FindPaymentAcquirerOnboardingWizardId(criteria *Criteria, options *Options) (int64, error) {
	...
	return -1, fmt.Errorf("payment.acquirer.onboarding.wizard was not found with criteria %v and options %v", criteria, options)
}
