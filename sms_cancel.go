...
func (c *Client) CreateSmsCancel(sc *SmsCancel) (int64, error) {
	ids, err := c.CreateSmsCancels([]*SmsCancel{sc})
	if err != nil {
		return -1, err
	}
	if len(ids) == 0 {
...
// CreateSmsCancel creates a new sms.cancel model and returns its id.
func (c *Client) CreateSmsCancels(scs []*SmsCancel) ([]int64, error) {
	var vv []interface{}
	for _, v := range scs {
		vv = append(vv, v)
	}
	return c.Create(SmsCancelModel, vv)
}
...
func (c *Client) FindSmsCancel(criteria *Criteria) (*SmsCancel, error) {
	...
	return nil, fmt.Errorf("sms.cancel was not found with criteria %v", criteria)
}
...
func (c *Client) FindSmsCancelId(criteria *Criteria, options *Options) (int64, error) {
	...
	return -1, fmt.Errorf("sms.cancel was not found with criteria %v and options %v", criteria, options)
}
