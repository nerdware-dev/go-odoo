...
func (c *Client) CreateIrQwebFieldMany2Many(iqfm *IrQwebFieldMany2Many) (int64, error) {
	ids, err := c.CreateIrQwebFieldMany2Manys([]*IrQwebFieldMany2Many{iqfm})
	if err != nil {
		return -1, err
	}
	if len(ids) == 0 {
...
// CreateIrQwebFieldMany2Many creates a new ir.qweb.field.many2many model and returns its id.
func (c *Client) CreateIrQwebFieldMany2Manys(iqfms []*IrQwebFieldMany2Many) ([]int64, error) {
	var vv []interface{}
	for _, v := range iqfms {
		vv = append(vv, v)
	}
	return c.Create(IrQwebFieldMany2ManyModel, vv)
}
...
func (c *Client) FindIrQwebFieldMany2Many(criteria *Criteria) (*IrQwebFieldMany2Many, error) {
	...
	return nil, fmt.Errorf("ir.qweb.field.many2many was not found with criteria %v", criteria)
}
...
func (c *Client) FindIrQwebFieldMany2ManyId(criteria *Criteria, options *Options) (int64, error) {
	...
	return -1, fmt.Errorf("ir.qweb.field.many2many was not found with criteria %v and options %v", criteria, options)
}
