...
func (c *Client) CreateAccountAnalyticDistribution(aad *AccountAnalyticDistribution) (int64, error) {
	ids, err := c.CreateAccountAnalyticDistributions([]*AccountAnalyticDistribution{aad})
	if err != nil {
		return -1, err
	}
	if len(ids) == 0 {
...
// CreateAccountAnalyticDistribution creates a new account.analytic.distribution model and returns its id.
func (c *Client) CreateAccountAnalyticDistributions(aads []*AccountAnalyticDistribution) ([]int64, error) {
	var vv []interface{}
	for _, v := range aads {
		vv = append(vv, v)
	}
	return c.Create(AccountAnalyticDistributionModel, vv)
}
...
func (c *Client) FindAccountAnalyticDistribution(criteria *Criteria) (*AccountAnalyticDistribution, error) {
	...
	return nil, fmt.Errorf("account.analytic.distribution was not found with criteria %v", criteria)
}
...
func (c *Client) FindAccountAnalyticDistributionId(criteria *Criteria, options *Options) (int64, error) {
	...
	return -1, fmt.Errorf("account.analytic.distribution was not found with criteria %v and options %v", criteria, options)
}
