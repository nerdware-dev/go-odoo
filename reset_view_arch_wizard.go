...
func (c *Client) CreateResetViewArchWizard(rvaw *ResetViewArchWizard) (int64, error) {
	ids, err := c.CreateResetViewArchWizards([]*ResetViewArchWizard{rvaw})
	if err != nil {
		return -1, err
	}
	if len(ids) == 0 {
...
// CreateResetViewArchWizard creates a new reset.view.arch.wizard model and returns its id.
func (c *Client) CreateResetViewArchWizards(rvaws []*ResetViewArchWizard) ([]int64, error) {
	var vv []interface{}
	for _, v := range rvaws {
		vv = append(vv, v)
	}
	return c.Create(ResetViewArchWizardModel, vv)
}
...
func (c *Client) FindResetViewArchWizard(criteria *Criteria) (*ResetViewArchWizard, error) {
	...
	return nil, fmt.Errorf("reset.view.arch.wizard was not found with criteria %v", criteria)
}
...
func (c *Client) FindResetViewArchWizardId(criteria *Criteria, options *Options) (int64, error) {
	...
	return -1, fmt.Errorf("reset.view.arch.wizard was not found with criteria %v and options %v", criteria, options)
}
