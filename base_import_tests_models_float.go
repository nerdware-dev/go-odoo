...
func (c *Client) CreateBaseImportTestsModelsFloat(btmf *BaseImportTestsModelsFloat) (int64, error) {
	ids, err := c.CreateBaseImportTestsModelsFloats([]*BaseImportTestsModelsFloat{btmf})
	if err != nil {
		return -1, err
	}
	if len(ids) == 0 {
...
// CreateBaseImportTestsModelsFloat creates a new base_import.tests.models.float model and returns its id.
func (c *Client) CreateBaseImportTestsModelsFloats(btmfs []*BaseImportTestsModelsFloat) ([]int64, error) {
	var vv []interface{}
	for _, v := range btmfs {
		vv = append(vv, v)
	}
	return c.Create(BaseImportTestsModelsFloatModel, vv)
}
...
func (c *Client) FindBaseImportTestsModelsFloat(criteria *Criteria) (*BaseImportTestsModelsFloat, error) {
	...
	return nil, fmt.Errorf("base_import.tests.models.float was not found with criteria %v", criteria)
}
...
func (c *Client) FindBaseImportTestsModelsFloatId(criteria *Criteria, options *Options) (int64, error) {
	...
	return -1, fmt.Errorf("base_import.tests.models.float was not found with criteria %v and options %v", criteria, options)
}
