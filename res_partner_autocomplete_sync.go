...
type ResPartnerAutocompleteSync struct {
	...
	Synched     *Bool     `xmlrpc:"synched,omitempty" json:"synched,omitempty"`
...
func (c *Client) CreateResPartnerAutocompleteSync(rpas *ResPartnerAutocompleteSync) (int64, error) {
	ids, err := c.CreateResPartnerAutocompleteSyncs([]*ResPartnerAutocompleteSync{rpas})
	if err != nil {
		return -1, err
	}
	if len(ids) == 0 {
...
// CreateResPartnerAutocompleteSync creates a new res.partner.autocomplete.sync model and returns its id.
func (c *Client) CreateResPartnerAutocompleteSyncs(rpass []*ResPartnerAutocompleteSync) ([]int64, error) {
	var vv []interface{}
	for _, v := range rpass {
		vv = append(vv, v)
	}
	return c.Create(ResPartnerAutocompleteSyncModel, vv, nil)
}
...
func (c *Client) UpdateResPartnerAutocompleteSyncs(ids []int64, rpas *ResPartnerAutocompleteSync) error {
	return c.Update(ResPartnerAutocompleteSyncModel, ids, rpas, nil)
}
...
func (c *Client) FindResPartnerAutocompleteSyncIds(criteria *Criteria, options *Options) ([]int64, error) {
	return c.Search(ResPartnerAutocompleteSyncModel, criteria, options)
}
