...
type DigestDigest struct {
	...
	KpiAccountBankCash                 *Bool      `xmlrpc:"kpi_account_bank_cash,omptempty"`
	KpiAccountBankCashValue            *Float     `xmlrpc:"kpi_account_bank_cash_value,omptempty"`
	...
	KpiHelpdeskTicketsClosed           *Bool      `xmlrpc:"kpi_helpdesk_tickets_closed,omptempty"`
	KpiHelpdeskTicketsClosedValue      *Int       `xmlrpc:"kpi_helpdesk_tickets_closed_value,omptempty"`
...
func (c *Client) CreateDigestDigests(dds []*DigestDigest) ([]int64, error) {
	...
	return c.Create(DigestDigestModel, vv)
}
...
func (c *Client) FindDigestDigest(criteria *Criteria) (*DigestDigest, error) {
	...
	return nil, fmt.Errorf("digest.digest was not found with criteria %v", criteria)
}
...
func (c *Client) FindDigestDigestId(criteria *Criteria, options *Options) (int64, error) {
	...
	return -1, fmt.Errorf("digest.digest was not found with criteria %v and options %v", criteria, options)
}
